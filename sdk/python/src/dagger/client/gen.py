...
class Agent(Type):
    ...
    async def resume(self) -> Self:
        ...
        Resuming a FAILED agent retries: the loop relaunches from the last
        committed snapshot, whose still-pending input is stepped again.
    ...
    async def send(self, message: str) -> str:
        ...
        Sending to a paused or failed agent enqueues with QUEUED delivery, to
        be drained by a resume. Sending to a stopped agent fails: nothing
        would ever consume the message.
    ...
    async def wait_for(
        ...
        *,
        ...
        Fails if the state becomes unreachable: STOPPED is the only terminal
        state — waiting on a FAILED agent for another state blocks, since a
        resume may retry the loop.
