...
// readable). Called whenever the conversation's LLM is replaced wholesale --
// model change, branch, clear, compact, resume, workspace rebind -- since the
// old instance's seed no longer matches: the next prompt submit spawns a fresh
// agent from the new value.
...
// go through setLLM instead. Since a different value digest is a different
// agent runtime instance by design, any live agent is dropped (and stopped, if
// owned) here; the next prompt submit packages the new value as a fresh agent.
...
func (a *sessionAgent) setLLM(llm *dagger.LLM) error {
	...
	// current without each call site having to remember to refresh it.
	return a.updateStatusLine(a.llm)
}
...
func (a *sessionAgent) updateChangesPreview(llm *dagger.LLM) error {
	...
	changes, err := idtui.PreviewWorkspaceChanges(a.session.plumbingCtx, a.session.dag, llm.Workspace())
	if err != nil {
		return err
	}
	if changes.Empty() {
...
func (a *sessionAgent) ExportChanges(ctx context.Context) error {
	...
	rebound, err := a.llm.WithWorkspace(a.session.dag.CurrentWorkspace()).Sync(ctx)
	if err != nil {
...
func (a *sessionAgent) ResetWorkspace(ctx context.Context) error {
	...
	reset, err := a.llm.WithWorkspace(a.session.dag.CurrentWorkspace().Reloaded()).Sync(ctx)
	if err != nil {
